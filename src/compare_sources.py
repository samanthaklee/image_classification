...
t0 = time.time()
resnet_source = applications.ResNet50(weights='imagenet', pooling='max', include_top=False)
t1 = time.time()
...
t0 = time.time()
nasnet_source = applications.NASNetMobile(weights='imagenet',pooling='max', include_top=False)
t1 = time.time()
...
hists = [
    vgg_hist,
     inception_hist, resnet_hist, mobilenet_hist, nasnet_hist]
names = [
    'vgg',
...
for i in zip(names, hists):
    name = i[0]
    hist = i[1].history
    path = 'results/sources/n200_10epochs/{}_hist.p'.format(name)
    print(type(hist))
